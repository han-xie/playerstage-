                                      ...
                                      void* data)
{
	...
	else if(Message::MatchMessage(hdr, PLAYER_MSGTYPE_CMD, PLAYER_GRIPPER_CMD_CLOSE, this->addr))
	{
		gmod->CommandClose();
		return 0;
	}else if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_CMD,PLAYER_GRIPPER_CMD_STORE, this->addr)) {
		gmod->CommandStore();
		return 0;
	}
	else if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_CMD,PLAYER_GRIPPER_CMD_RETRIEVE, this->addr)) {
