 ...
 * CVS: $Id: p_driver.cc,v 1.2 2008-01-15 01:25:42 rtv Exp $
 */
...
Interface::Interface(player_devaddr_t addr, StgDriver* driver, ConfigFile* cf,
		int section) {
...
InterfaceModel::InterfaceModel(player_devaddr_t addr, StgDriver* driver,
		ConfigFile* cf, int section, const std::string& type) :
...
StgDriver::StgDriver(ConfigFile* cf, int section) :
		Driver(cf, section, false, 4096), devices() {
//	initialtimes++;
//	printf("---init times:%d\n", initialtimes);
	...
	for (int d = 0; d < device_count; d++) {
		...
		if (!player_quiet_startup) {
			printf(" Stage plugin:  %d.%s.%d is ", player_addr.robot,
					interf_to_str(player_addr.interf), player_addr.index);
			fflush (stdout);
		}
...
int StgDriver::Shutdown() {
	...
	FOR_EACH(it, this->devices)(*it)->Unsubscribe();
...
int StgDriver::ProcessMessage(QueuePointer &resp_queue, player_msghdr * hdr,
		void * data) {
	...
	if (in) {
		...
		//maybe a broadcast message from requires
		FOR_EACH(it, this->devices)
		{
			Interface* candidate = *it;
			if (candidate->addr.interf == hdr->addr.interf)
				return (candidate->ProcessMessage(resp_queue, hdr, data));
		}
