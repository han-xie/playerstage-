...
int InterfaceSimulation::ProcessMessage(QueuePointer &resp_queue,
	...
	if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_REQ,
			PLAYER_SIMULATION_REQ_GET_POSE2D, this->addr)) {
		...
		if (mod) {
			...
			PRINT_WARN1(
					"Stage: GET_POSE2D request: simulation model \"%s\" not found",
					req->name);
			return (-1);
		}
	}
	...
	if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_REQ,
			PLAYER_SIMULATION_REQ_SET_POSE2D, this->addr)) {
		player_simulation_pose2d_req_t* req =
				(player_simulation_pose2d_req_t*) data;
	...
	// Is it a request to get a model's pose in 3D?
			...
			PLAYER_SIMULATION_REQ_GET_POSE3D, this->addr)) {
		...
		if (mod) {
			...
			PRINT_WARN1(
					"Stage: GET_POSE3D request: simulation model \"%s\" not found",
					req->name);
			return (-1);
		}
	}
	...
	if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_REQ,
			PLAYER_SIMULATION_REQ_SET_POSE3D, this->addr)) {
		player_simulation_pose3d_req_t* req =
				(player_simulation_pose3d_req_t*) data;
		...
		PRINT_WARN2(
				"stg_simulation doesn't support msg with type/subtype %d/%d",
