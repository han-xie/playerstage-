...
InterfacePosition::InterfacePosition(player_devaddr_t addr, StgDriver* driver,
	...
	//opaquem.positionModel = (ModelPosition*) this->mod;
	...
	else if (pos2dType == "FPOS2D8BUTTON")
		this->conf.type = FPOS2D8BUTTON;
	else if (pos2dType == "FPOS2DOMNIDIR")
		this->conf.type = FPOS2DOMNIDIR;
	else if (pos2dType == "FPOS2D4TIRES")
		this->conf.type = FPOS2D4TIRES;
	this->conf.redressAngle = 0;

	memset(&this->require_addr, 0, sizeof(player_devaddr_t));
	if (cf->ReadDeviceAddr(&this->require_addr, section, "requires",
			PLAYER_POSITION2D_CODE, -1, NULL) == 0) {
		this->require_dev = NULL;
		if (this->require_dev = deviceTable->GetDevice(this->require_addr)) {
			if (this->require_dev->Subscribe(this->driver->InQueue) != 0) {
				printf("unable to subscribe to laser device\n");
			}
		} else
			printf("can't find the Device\n");
	} else
		printf("missing field requires\n");
...
int InterfacePosition::ProcessMessage(QueuePointer &resp_queue,
	...
	if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_DATA,
		...
		if (this->conf.type == FPOS2D8BUTTON) {
			...
			//mod->GoTo(pcmd->pos2d.pos.px, pcmd->pos2d.pos.py,pcmd->pos2d.pos.pa - pcmd->redressAngle);
			Pose pose;
			pose.x = pcmd->pos2d.pos.px;
			pose.y = pcmd->pos2d.pos.py;
			pose.a = pcmd->pos2d.pos.pa - pcmd->redressAngle;
			mod->SetPose(pose);
			return 0;
		}
	...
	// Is it a new motor command?
			...
			PLAYER_POSITION2D_CMD_VEL, this->addr)) {
	...
	// Is it a new motor command?
			...
			PLAYER_POSITION2D_CMD_POS, this->addr)) {
		// convert from Player to Stage format
		player_position2d_cmd_pos_t* pcmd = (player_position2d_cmd_pos_t*) data;

		mod->GoTo(pcmd->pos.px, pcmd->pos.py, pcmd->pos.pa);
		if (this->conf.type == FPOS2D8BUTTON) {
			this->conf.redressAngle = pcmd->pos.pa;
		}
		return 0;
	}
	...
	// Is it a new motor command?
			...
			PLAYER_POSITION2D_CMD_CAR, this->addr)) {
		// convert from Player to Stage format
	...
	// Is it a request for position geometry?
			...
			PLAYER_POSITION2D_REQ_GET_GEOM, this->addr)) {
		if (hdr->size == 0) {
	...
	// Is it a request to reset odometry?
			...
			PLAYER_POSITION2D_REQ_RESET_ODOM, this->addr)) {
		if (hdr->size == 0) {
	...
	// Is it a request to set odometry?
			...
			PLAYER_POSITION2D_REQ_SET_ODOM, this->addr)) {
	...
	// Is it a request to enable motor power?
			...
			PLAYER_POSITION2D_REQ_MOTOR_POWER, this->addr)) {
	...
	// Is it a request to switch control mode?
			...
			PLAYER_POSITION2D_REQ_POSITION_MODE, this->addr)) {
		if (hdr->size == sizeof(player_position2d_position_mode_req_t)) {
			...
			PRINT_WARN2( "Put model %s into %s control mode",
					this->mod->Token(), mod ? "POSITION" : "VELOCITY");
...
void InterfacePosition::Publish(void) {
	...
	if (this->conf.type == POS2D8BUTTON) {
		player_position2d_data_t_redress ppdr;
		ppdr.pos2d = ppd;
		ppdr.redressAngle = this->conf.redressAngle;
		this->driver->Publish(this->addr, PLAYER_MSGTYPE_DATA,
				PLAYER_POSITION2D_DATA_STATE, (void*) &ppdr, sizeof(ppdr),
				NULL);
		return;
	}
