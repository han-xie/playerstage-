...
void Model::Visibility::Load(Worldfile* wf, int wf_entity) {
	...
	fiducial_return = wf->ReadInt(wf_entity, "fiducial_return",
			fiducial_return);
...
Model::GuiState::GuiState() :
		grid(false), move(false), nose(false), outline(false) { /* nothing to do */
}
...
// constructor
Model::Model(World* world, Model* parent, const std::string& type) :
		Ancestor(), access_mutex(), alwayson(false), blockgroup(), blocks_dl(0), boundary(
				false),
		//  callbacks(),
		color(1, 0, 0), // red
		data_fresh(false), disabled(false), cv_list(), flag_list(), geom(), has_default_block(
				true), id(Model::count++), interval((stg_usec_t) 1e5), // 100msec
		interval_energy((stg_usec_t) 1e5), // 100msec
		interval_pose((stg_usec_t) 1e5), // 100msec
		last_update(0), log_state(false), map_resolution(0.1), mass(0), parent(
				parent), pose(), power_pack(NULL), pps_charging(), props(), rastervis(), rebuild_displaylist(
				true), say_string(), stall(false), subs(0), thread_safe(false), trail(
				trail_length), trail_index(0), type(type), event_queue_num(0), used(
				false), velocity(), velocity_enable(false), watts(0.0), watts_give(
				0.0), watts_take(0.0), wf(NULL), wf_entity(0), world(world), world_gui(
				dynamic_cast<WorldGui*>(world)) {
	//assert( modelsbyid );
	assert(world);
...
Block* Model::AddBlockRect(stg_meters_t x, stg_meters_t y, stg_meters_t dx,
		stg_meters_t dy, stg_meters_t dz) {
	UnMap();
	...
	Block* newblock = new Block(this, pts, 4, 0, dz, color, true);
...
stg_raytrace_result_t Model::Raytrace(const Pose &pose,
		const stg_meters_t range, const stg_ray_test_func_t func,
...
stg_raytrace_result_t Model::Raytrace(const stg_radians_t bearing,
	...
	return world->Raytrace(LocalToGlobal(raystart), range, func, this, arg,
			ztest);
}

void Model::Raytrace(const stg_radians_t bearing, const stg_meters_t range,
		const stg_radians_t fov, const stg_ray_test_func_t func,
		const void* arg, stg_raytrace_result_t* samples,
		const uint32_t sample_count, const bool ztest) {
	Pose raystart;
	bzero(&raystart, sizeof(raystart));
	raystart.a = bearing;

	world->Raytrace(LocalToGlobal(raystart), range, fov, func, this, arg,
			samples, sample_count, ztest);
}
...
void Model::LocalToPixels(const std::vector<stg_point_t>& local,
	...
	FOR_EACH( it, local )
	{
		...
		global.push_back(
				stg_point_int_t((int32_t) floor(ptpose.x * world->ppm),
...
const char* Model::PrintWithPose() const {
	...
	static char txt[256];
	snprintf(txt, sizeof(txt), "%s @ [%.2f,%.2f,%.2f,%.2f]", token.c_str(),
...
void Model::UpdatePose(void) {
	...
	Pose p(velocity.x * interval, velocity.y * interval, velocity.z * interval,
...
Model* Model::GetUnusedModelOfType(const std::string& type) {
	...
	// nothing matching below this model
	if (!parent)
		PRINT_WARN1( "Request for unused model of type %s failed",
				type.c_str());
	return NULL;
}
...
Model::RasterVis::RasterVis() :
		Visualizer("Rasterization", "raster_vis"), data(NULL), width(0), height(
				0), cellwidth(0), cellheight(0), pts() {
...
void Model::RasterVis::SetData(uint8_t* data, unsigned int width,
		unsigned int height, stg_meters_t cellwidth, stg_meters_t cellheight) {
...
int Model::GetCYZXPort(const char *portname) {
	if (portname == NULL)
		return 0;
	if (strcmp(portname, "a0") == 0)
		return 0;
	else if (strcmp(portname, "a1") == 0)
		return 1;
	else if (strcmp(portname, "a2") == 0)
		return 2;
	else if (strcmp(portname, "a3") == 0)
		return 3;
	else if (strcmp(portname, "a4") == 0)
		return 4;
	else if (strcmp(portname, "a5") == 0)
		return 5;
	else if (strcmp(portname, "a6") == 0)
		return 6;
	else if (strcmp(portname, "a7") == 0)
		return 7;
	else if (strcmp(portname, "d0") == 0)
		return 8;
	else if (strcmp(portname, "d1") == 0)
		return 9;
	else if (strcmp(portname, "d2") == 0)
		return 10;
	else if (strcmp(portname, "d3") == 0)
		return 11;
	else if (strcmp(portname, "d4") == 0)
		return 12;
	else if (strcmp(portname, "d5") == 0)
		return 13;
	else if (strcmp(portname, "d6") == 0)
		return 14;
	else if (strcmp(portname, "d7") == 0)
		return 15;
	else if (strcmp(portname, "d8") == 0)
		return 16;
	else if (strcmp(portname, "d9") == 0)
		return 17;
	else if (strcmp(portname, "d10") == 0)
		return 18;
	else if (strcmp(portname, "d11") == 0)
		return 19;
	else if (strcmp(portname, "rs4220") == 0)
		return 20;
	else if (strcmp(portname, "rs4221") == 0)
		return 21;
	return 0;
}
