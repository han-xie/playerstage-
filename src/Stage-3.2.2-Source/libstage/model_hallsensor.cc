///////////////////////////////////////////////////////////////////////////
//
// File: model_hallsensor.c
// Author: Richard Vaughan
// Date: 10 June 2004
//
// CVS info:
//  $Source: /home/tcollett/stagecvs/playerstage-cvs/code/stage/libstage/model_hallsensor.cc,v $
//  $Author: rtv $
//  $Revision: 1.3 $
//
///////////////////////////////////////////////////////////////////////////
...
static const stg_radians_t DEFAULT_HALLSENSORPAN = 0.0;
static const unsigned int DEFAULT_HALLSENSORINTERVAL_MS = 100;
...
/**
  @ingroup model
  @defgroup model_hallsensor Hallsensor model

  The hallsensor model simulates a color-hall-finding vision device,
...
- image [ width:<int> height:<int> ]\n
  dimensions of the image in pixels. This determines the hallsensor's resolution
- range <float>\n
 ...
 */
  ...
  ModelHallsensor::ModelHallsensor( World* world,
												Model* parent,
  ...
  Model( world, parent, type ),
						...
						halls(),
						colors(),
						fov( DEFAULT_HALLSENSORFOV ),
						pan( DEFAULT_HALLSENSORPAN ),
						range( DEFAULT_HALLSENSORRANGE ),
						scan_height( DEFAULT_HALLSENSORSCANHEIGHT ),
						scan_width( DEFAULT_HALLSENSORSCANWIDTH )
{
  /*PRINT_DEBUG2( "Constructing ModelHallsensor %d (%s)\n",
					 id, typestr );*/
	  PRINT_DEBUG2( "Constructing ModelHallsensor %d (%s)\n",
	  					 id, type.data() );
  ClearBlocks();
...
ModelHallsensor::~ModelHallsensor( void )
{
}
...
{ 
  ...
  if(candidate->vis.hall_return==false) return false;
...
void ModelHallsensor::ModelHallsensor::AddColor( Color col )
{
	colors.push_back( col );
}

/** Stop tracking halls with this color */
void ModelHallsensor::RemoveColor( Color col )
{
	FOR_EACH( it, colors )
		{
			if( (*it) ==  col  )
...
void ModelHallsensor::RemoveAllColors()
{
	colors.clear();
}

void ModelHallsensor::Load( void )
{  
	Model::Load();
	...
	port=Model::GetCYZXPort(portname.data());
...
void ModelHallsensor::Startup(  void )
{ 
	Model::Startup();

	PRINT_DEBUG( "hallsensor startup" );

	// start consuming power
	SetWatts( DEFAULT_HALLSENSORWATTS );
}

void ModelHallsensor::Shutdown( void )
{ 

	PRINT_DEBUG( "hallsensor shutdown" );
...
// Option ModelHallsensor::showHallData( "Show Hallsensor", "show_hall", "", true, NULL );

ModelHallsensor::Vis::Vis( World* world )
  : Visualizer( "Hallsensor", "hallsensor_vis" )
{
...
void ModelHallsensor::Vis::Visualize( Model* mod, Camera* cam )
{
  ModelHallsensor* bf( dynamic_cast<ModelHallsensor*>(mod) );
  ...
  for( unsigned int s=0; s<bf->halls.size(); s++ )
	 {
		Hall* b = &bf->halls[s];
		//stg_hallsensor_hall_t* b =
		//&g_array_index( halls, stg_hallsensor_hall_t, s);
