...
ModelGripper::ModelGripper(World* world, Model* parent, const std::string& type) :
...
void ModelGripper::Load() {
	...
	cfg.paddle_size.x = wf->ReadTupleFloat(wf_entity, "paddle_size", 0,
			cfg.paddle_size.x);
...
void ModelGripper::Save() {
	...
	wf->WriteTupleString(wf_entity, "paddle_state", 0,
			(cfg.paddles == PADDLE_CLOSED) ? "closed" : "open");
	wf->WriteTupleString(wf_entity, "paddle_state", 1,
...
void ModelGripper::FixBlocks() {
	...
	// left (top) paddle
	paddle_left = AddBlockRect(1.0 - cfg.paddle_size.x, 0, cfg.paddle_size.x,
			cfg.paddle_size.y, cfg.paddle_size.z);

	// right (bottom) paddle
	paddle_right = AddBlockRect(1.0 - cfg.paddle_size.x,
			1.0 - cfg.paddle_size.y, cfg.paddle_size.x, cfg.paddle_size.y,
			cfg.paddle_size.z);
...
static bool gripper_raytrace_match(Model* hit, Model* finder,
		const void* dummy) {
...
void ModelGripper::UpdateBreakBeams() {
	for (unsigned int index = 0; index < 2; index++) {
		...
		double bbr = (1.0 - cfg.paddle_position)
				* (geom.size.y - (geom.size.y * cfg.paddle_size.y * 2.0));

		stg_raytrace_result_t sample = Raytrace(pz,  // ray origin
				bbr, // range
...
//   //pz.x = (geom.size.x - inset * geom.size.x) - geom.size.x/2.0;
	...
	stg_raytrace_result_t leftsample = Raytrace(lpz,  // ray origin
			bbr, // range
			gripper_raytrace_match, // match func
			NULL, // match arg
			true); // ztest
	...
	stg_raytrace_result_t rightsample = Raytrace(rpz,  // ray origin
			bbr, // range
			gripper_raytrace_match, // match func
			NULL, // match arg
			true); // ztest
	...
	if (cfg.contact[0] || cfg.contact[1]) {
		...
		if (cfg.paddles == PADDLE_CLOSING) {
			...
			if (cfg.gripped == NULL) // if we're not carrying something already
				...
				cfg.close_limit = std::max(0.0,
						1.0 - puckw / (gripperw - cfg.paddle_size.y / 2.0));
			}
		}
	}
}
...
void ModelGripper::CommandStore() {
	Pose tp;
	tp.x = 4;
	tp.y = 4;
	tp.z = 0;
	...
	bool beam2 = false;
	if (cfg.beam[0]) {
		cfg.store[cfg.storeV] = cfg.beam[0];
		cfg.storeV++;
		beam1 = true;
		//this->Model::world->RemoveModel(cfg.beam[0]);
		//cfg.beam[0]->SetPose(tp);
	}
	if (cfg.beam[1]) {
		cfg.store[cfg.storeV] = cfg.beam[1];
		cfg.storeV++;
		beam2 = true;
		//this->Model::world->RemoveModel(cfg.beam[1]);
		//cfg.beam[1]->SetPose(tp);
	}
	CommandOpen();
	if (beam1) {
		this->Model::world->RemoveModel(cfg.beam[0]);
	...
	this->Model::world->GetModel("hospital")->NeedRedraw();
}

void ModelGripper::CommandTest() {
	Pose tp;
	tp.x = 0;
	tp.y = 0;
	tp.z = 0;
	...
	int j;
	printf(
			"0 exit,1 SetPose,2 SetGeom,3 SetGlobalPose,4 GetGeom,5 GetPose,6 AddToPose\n");
	/*
	...
	for (;;) {
		...
		case 1: {
			if (cfg.beam[0])
				...
				j = 2;
			if (j == 2) {
				printf("blobfinder can not find thing\n");
				break;
			}
			printf("x,y,z\n");
			tp.x++;
		...
		case 4: {
			...
			tg = cfg.beam[j]->GetGeom();
			printf("%d,%d,%d\n", tg.pose.x, tg.pose.y, tg.pose.z);
			break;
		}
		case 5: {
			if (cfg.beam[0])
