 ...
 * CVS: $Id: p_gripper.cc,v 1.2 2008-01-15 01:25:42 rtv Exp $
 */
...
InterfaceGripper::InterfaceGripper(player_devaddr_t addr, StgDriver* driver,
...
void InterfaceGripper::Publish(void) {
	...
	this->driver->Publish(this->addr, PLAYER_MSGTYPE_DATA,
			PLAYER_GRIPPER_DATA_STATE, (void*) &pdata);
...
int InterfaceGripper::ProcessMessage(QueuePointer& resp_queue,
	...
	if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_CMD, PLAYER_GRIPPER_CMD_OPEN,
			this->addr)) {
		gmod->CommandOpen();
		return 0;
	} else if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_CMD,
			PLAYER_GRIPPER_CMD_CLOSE, this->addr)) {
		gmod->CommandClose();
		return 0;
	} else if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_CMD,
			PLAYER_GRIPPER_CMD_STORE, this->addr)) {
		gmod->CommandStore();
		return 0;
	} else if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_CMD,
			PLAYER_GRIPPER_CMD_RETRIEVE, this->addr)) {
		gmod->CommandTest();
		return 0;
	} else if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_CMD,
			PLAYER_GRIPPER_CMD_STOP, this->addr)) {
		gmod->CommandTest();
		return 0;
	}

	if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_REQ,
			PLAYER_GRIPPER_REQ_GET_GEOM, this->addr)) {
	...
	PRINT_WARN2( "stage gripper doesn't support message id:%d/%d", hdr->type,
			hdr->subtype);
	return -1;
}
