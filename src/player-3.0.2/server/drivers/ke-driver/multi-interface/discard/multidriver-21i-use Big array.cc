...
public:
	...
	virtual int ProcessMessage(QueuePointer &resp_queue, player_msghdr * hdr,
			void * data);
	static void setFrontL(int fl) {
		this->frontl = fl;
	}
	static void setFrontR(int fr) {
		this->frontr = fr;
	}
	static void setRearL(int rl) {
		this->behindl = rl;
	}
	static void setRearR(int rr) {
		this->behindr = rr;
	}
	static int getFrontL() {
		return this->frontl;
	}
	static int getfrontR() {
		return this->frontr;
	}
	static int getRearL() {
		return this->behindl;
...
private:
	...
	//the ptr to function
	void ((*fDealAllComReq[MAXCOMMANDNUMBER])(QueuePointer & resp_queue,
...
multidriver::multidriver(ConfigFile* cf, int section) :
	...
	this->fDealAllComReq[SRSTARTF] = multidriver::CYZXSRSTARTF;
...
// return -1, and a NACK will be sent for you, if a response is required.
	...
	if (hdr->addr.interf == this->m_opaque_addr.interf) {
		...
		this->opaquemc++;
#endif
		myOpaqueHead *popaqueHI = (myOpaqueHead*) data;
		if ((this->fDealAllComReq)[popaqueHI->subtype])
			((this->fDealAllComReq)[popaqueHI->subtype])(resp_queue, hdr, data);
