...
private:
	...
	int dealOpaqueComReq(my_opaque_st *popa, my_opaque_st *pr);
...
int multidriver::dealOpaqueComReq(my_opaque_st *popa, my_opaque_st *pr) {
	pr->type = popa->type;
...
#ifdef WRITELOG
	printf("comming opaque ComReq : %d %d %d %d \n", popa->type, popa->subtype,
			popa->p1, popa->p2);
	printf("outing opaque message : %d %d %d %d\n", pr->type, pr->subtype, pr->p1,
			pr->p2);
#endif
	return 0;
}
int multidriver::dealOpaqueReq(my_opaque_st *popa, my_opaque_st *pr) {
	...
	printf("comming opaque req : %d %d %d %d \n", popa->type, popa->subtype,
			popa->p1, popa->p2);
	printf("outing opaque message : %d %d %d %d\n", pr->type, pr->subtype, pr->p1,pr->p2);
#endif
	return 0;
}
