...
/*
 * $Id: configfile.cc 8004 2009-07-13 14:03:44Z thjc $
 */
...
const char * COLOR_DATABASE[5] = { "/usr/X11R6/lib/X11/rgb.txt",
		"/usr/share/X11/rgb.txt", "/etc/X11/rgb.txt", "/usr/lib/X11/rgb.txt",
		NULL };
...
void ConfigFile::InitFields() {
	...
	if (!setlocale(LC_ALL, "POSIX"))
		fputs(
				"Warning: failed to setlocale(); config file may not be parse correctly\n",
				stderr);
}
...
bool ConfigFile::Load(const char *filename) {
	...
	if (!file) {
		PLAYER_ERROR2("unable to open world file %s : %s", this->filename,
				strerror(errno));
		//fclose(file);
		return false;
	}
...
bool ConfigFile::Save(const char *filename) {
	...
	if (!file) {
		PLAYER_ERROR2("unable to open world file %s : %s", filename,
				strerror(errno));
		return false;
	}
...
bool ConfigFile::LoadTokenInclude(FILE *file, int *line, int include) {
	...
	if (!infile) {
		PLAYER_ERROR2("unable to open include file %s : %s", fullpath,
				strerror(errno));
		free(fullpath);
		return false;
	}
...
int ConfigFile::AddMacro(const char *macroname, const char *sectionname,
	...
	if (this->macro_count >= this->macro_size) {
		this->macro_size += 100;
		this->macros = (CMacro*) realloc(this->macros,
				this->macro_size * sizeof(this->macros[0]));
	}
...
int ConfigFile::AddSection(int parent, const char *type) {
	if (this->section_count >= this->section_size) {
		...
		this->sections = (Section*) realloc(this->sections,
				this->section_size * sizeof(this->sections[0]));
	}
...
void ConfigFile::AddFieldValue(int field, int index, int value_token) {
	...
	if (index >= pfield->value_count) {
		...
		pfield->values = (int*) realloc(pfield->values,
				pfield->value_count * sizeof(int));
		pfield->useds = (bool*) realloc(pfield->useds,
				pfield->value_count * sizeof(bool));
...
bool ConfigFile::ReadBool(int section, const char* name, bool value) {
	...
	if (strcmp(s, "yes") == 0 || strcmp(s, "true") == 0 || strcmp(s, "1") == 0)
		...
		printf(
				"player: Warning: error in config file section %d field %s: Invalid boolean value.",
				section, name);
		return value;
	}
}
...
void ConfigFile::WriteLength(int section, const char *name, double value) {
	char default_str[64];
	snprintf(default_str, sizeof(default_str), "%.3f",
...
uint32_t ConfigFile::LookupColor(const char *name) {
	...
	if (!file) {
		PLAYER_ERROR(
				"unable to open color database: tried the following files");
		for (int i = 0; COLOR_DATABASE[i] != NULL; ++i) {
...
int ConfigFile::ReadDeviceAddr(player_devaddr_t *addr, int section,
		const char *name, int interf_code, int index, const char *key) {
	int prop;
	int i, j, count;
	char str[128];
	char *tokens[5];
	...
	for (i = 0; i < count; i++) {
		...
		if (!(tokens[3] && tokens[4])) {
			CONFIG_ERR1("missing interface or index in field [%s]",
					this->fields[prop].line, name);
			return -1;
		}
...
bool ConfigFile::ParseAllDrivers() {
...
bool ConfigFile::ParseDriver(int section) {
	...
	if (entry->initfunc == NULL) {
		PLAYER_ERROR1("Driver has no initialization function \"%s\"",
				drivername);
		return (false);
	}
	...
	count = 0;
	for (device = deviceTable->GetFirstDevice(); device != NULL; device =
			deviceTable->GetNextDevice(device)) {
	...
	if (driver)
		driver->alwayson =
				this->ReadInt(section, "alwayson", driver->alwayson) ?
						true : false;
...
bool ConfigFile::ParseAllInterfaces() {
	for (int i = 1; i < this->GetSectionCount(); i++) {
		// Check for new-style device block
...
bool ConfigFile::ParseInterface(int section) {
	const char *pluginname;
	...
	if (pluginname == NULL) {
		PLAYER_ERROR1(
				"No plugin name specified for plugin interface in section %d",
				section);
		return false;
	}
	...
	if (playerxdr_ftable_add_multi(flist, replace) < 0) {
		PLAYER_ERROR1(
				"Failed to add interface functions for plugin interface %s",
				interfacename);
		return false;
	}
