             ...
             PLAYER_POSITION2D_CODE)
{
  ...
  // Read an option from the configuration file
    if(cf->ReadDeviceAddr((player_devaddr_t *)&(this->position_id),section,"provides",PLAYER_POSITION2D_CODE,-1,NULL) == 0)
    {
...
int 
ExampleDriver::SetVelocity(double lvel, double rvel)
{
    MFSetServoRotaSpd(1,lvel);
    DelayMS(1);
    MFSetServoRotaSpd(4,lvel);
    DelayMS(1);
    MFSetServoRotaSpd(2,0-rvel);
    DelayMS(1);
    MFSetServoRotaSpd(3,0-rvel);
    DelayMS(1);
    MFServoAction();
...
int
ExampleDriver::ChangeMotorState(int state)
{
      if (!state) {
        SetVelocity (0, 0);
        this->_powered = false;
                                  ...
                                  void * data)
{
  ...
  if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_REQ, PLAYER_POSITION2D_REQ_GET_GEOM,*(player_devaddr_t *)&(this->position_id)))
  {
    ...
    Publish(*(player_devaddr_t *)&(this->position_id), resp_queue, PLAYER_MSGTYPE_RESP_ACK, PLAYER_POSITION2D_REQ_GET_GEOM, &geom, sizeof(geom),NULL);

    return 0;
  }

  if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_REQ, PLAYER_POSITION2D_REQ_MOTOR_POWER, *(player_devaddr_t *)&(this->position_id)))
  {
    ...
    if(ChangeMotorState(powercfg->state) < 0)
      Publish(*(player_devaddr_t *)&(this->position_id), resp_queue, PLAYER_MSGTYPE_RESP_NACK, PLAYER_POSITION2D_REQ_MOTOR_POWER);
    else
      Publish(*(player_devaddr_t *)&(this->position_id), resp_queue, PLAYER_MSGTYPE_RESP_ACK, PLAYER_POSITION2D_REQ_MOTOR_POWER);
    return 0;
  }

  if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_REQ, PLAYER_POSITION2D_REQ_SET_ODOM, *(player_devaddr_t *)&(this->position_id)))
  {
    ...
    while (this->_pa < -M_PI) this->_pa += M_PI;
    this->_resting = false;
    Publish (*(player_devaddr_t *)&(this->position_id), resp_queue, PLAYER_MSGTYPE_RESP_ACK, PLAYER_POSITION2D_REQ_SET_ODOM);
    return 0;
  }

  if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_REQ, PLAYER_POSITION2D_REQ_RESET_ODOM, *(player_devaddr_t *)&(this->position_id)))
  {
    this->_px = 0.0;
    this->_py = 0.0;
    this->_pa = 0.0;
    this->_resting = false;
    Publish (*(player_devaddr_t *)&(this->position_id), resp_queue, PLAYER_MSGTYPE_RESP_ACK, PLAYER_POSITION2D_REQ_RESET_ODOM);
    return 0;
  }

  if (Message::MatchMessage(hdr, PLAYER_MSGTYPE_CMD, PLAYER_POSITION2D_CMD_VEL, *(player_devaddr_t *)&(this->position_id)) ||
      Message::MatchMessage(hdr, PLAYER_MSGTYPE_CMD, PLAYER_POSITION2D_CMD_CAR, *(player_devaddr_t *)&(this->position_id)))
  {
