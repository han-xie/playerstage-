 ...
 * Date: 13 May 2002
 * CVS: $Id: dev_cyzxlaser.c 8004 2009-07-13 14:03:44Z thjc $
 **************************************************************************/
...
void playerc_cyzxlaser_putmsg(playerc_cyzxlaser_t *device,
                          player_msghdr_t *header,
                          void *data);

// Create a new cyzxlaser proxy
playerc_cyzxlaser_t *playerc_cyzxlaser_create(playerc_client_t *client, int index)
{
  playerc_cyzxlaser_t *device;

  device = malloc(sizeof(playerc_cyzxlaser_t));
  memset(device, 0, sizeof(playerc_cyzxlaser_t));
  playerc_device_init(&device->info, client, PLAYER_CYZXLASER_CODE, index,
                      (playerc_putmsg_fn_t) playerc_cyzxlaser_putmsg);
...
// Destroy a cyzxlaser proxy
void playerc_cyzxlaser_destroy(playerc_cyzxlaser_t *device)
{
...
// Subscribe to the cyzxlaser device
int playerc_cyzxlaser_subscribe(playerc_cyzxlaser_t *device, int access)
{
  return playerc_device_subscribe(&device->info, access);
}


// Un-subscribe from the cyzxlaser device
int playerc_cyzxlaser_unsubscribe(playerc_cyzxlaser_t *device)
{
  return playerc_device_unsubscribe(&device->info);
}

void playerc_cyzxlaser_reallocate_scans(playerc_cyzxlaser_t *device)
{
                          ...
                          void *data)
{
  ...
  if((header->type == PLAYER_MSGTYPE_DATA) &&
     (header->subtype == PLAYER_CYZXLASER_DATA_SCAN))
  {
    player_cyzxlaser_data_t* scan_data = (player_cyzxlaser_data_t*)data;
    ...
    playerc_cyzxlaser_reallocate_scans(device);
    ...
    device->scan_id = scan_data->id;
          ...
          (header->subtype == PLAYER_CYZXLASER_DATA_SCANPOSE))
  {
    player_cyzxlaser_data_scanpose_t* scan_data =
            (player_cyzxlaser_data_scanpose_t*)data;
    ...
    device->robot_pose[2] = scan_data->pose.pa;
          ...
          (header->subtype == PLAYER_CYZXLASER_DATA_SCANANGLE))
  {
	  player_cyzxlaser_data_scanangle_t* scan_data = (player_cyzxlaser_data_scanangle_t*)data;
    ...
    PLAYERC_WARN2("skipping cyzxlaser message with unknown type/subtype: %s/%d\n",
                 msgtype_to_str(header->type), header->subtype);
}


// Configure the cyzxlaser.
int
playerc_cyzxlaser_set_config(playerc_cyzxlaser_t *device,
                         double min_angle,
                         double max_angle,
                         double resolution,
                         double range_res,
...
{
  player_cyzxlaser_config_t config;
...
// Get the cyzxlaser configuration.
int
playerc_cyzxlaser_get_config(playerc_cyzxlaser_t *device,
                         double *min_angle,
                         double *max_angle,
                         double *resolution,
                         double *range_res,
                         ...
                         double *scanning_frequency)
{
  ...
  *scanning_frequency = config->scanning_frequency;
  ...
  player_cyzxlaser_config_t_free(config);
  return 0;
}

// Get the cyzxlaser geometry.  The writes the result into the proxy
// rather than returning it to the caller.
int
playerc_cyzxlaser_get_geom(playerc_cyzxlaser_t *device)
{
  player_cyzxlaser_geom_t *config;

  if(playerc_client_request(device->info.client,
                            &device->info,PLAYER_CYZXLASER_REQ_GET_GEOM,
                            NULL, (void**)&config) < 0)
    return -1;
  ...
  player_cyzxlaser_geom_t_free(config);

  return 0;
}

// Get the cyzxlaser IDentification information.  The writes the result into
// the proxy rather than returning it to the caller.
int
  playerc_cyzxlaser_get_id (playerc_cyzxlaser_t *device)
{
  player_cyzxlaser_get_id_config_t *config;

  if (playerc_client_request(device->info.client,
                            &device->info,PLAYER_CYZXLASER_REQ_GET_ID,
                            NULL, (void**)&config) < 0)
    return -1;

  device->cyzxlaser_id = config->serial_number;
  player_cyzxlaser_get_id_config_t_free(config);

  return 0;
}

void playerc_cyzxlaser_printout( playerc_cyzxlaser_t * device,
			  const char* prefix )
{
  int i;
  if( prefix )
